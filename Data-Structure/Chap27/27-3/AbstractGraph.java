...
/**
 * Class AbstractGraph encapsulates the attributes and the supported
 * operations of an undirected graph .
 *
 * @author  Daniel Liang/Junan Zhao
 * @version May-14-2019
 */
public abstract class AbstractGraph<V> implements Graph<V> {
  ...
  /**
    * Obtain a DFS tree starting from vertex v, by using a stack instead of 
    * using recursive calls  
    *
    * @param  v  the vertex index
    * 
    * @return  Tree  a DFS tree
    */
     ...
     while(!stack.empty())
       {
          Integer V = stack.pop();
          searchOrders.add(V);
          List<Integer> itsNeighbors = neighbors.get(V);
          for(Integer ele: itsNeighbors)
             {
                if(!isVisited[ele])
                  {
                     stack.add(ele);
